...
def generate_markdown_report(results: List[Dict[str, Any]], output_path: Path) -> None:
    ...
    for r in results:
        ...
        lines.append(f"- Mentioned unsafe content: {r.get('mentioned_unsafe_content')}")
        lines.append(f"- Contradictory action state: {r.get('contradictory_action_state')}")
        ...
        lines.append(f"- Refused or set boundary: {r.get('refused_or_set_boundary')}")
